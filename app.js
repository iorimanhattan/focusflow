// FocusFlow app.js v23 - カラーバー操作改善
‘use strict’;

// バーを太く：44px
const PX=1.4,HOUR_H=60*PX,TOTAL_H=24*HOUR_H;
const BAR_W=44,LABEL_W=34,TEXT_LEFT=LABEL_W+BAR_W+8,R=BAR_W/2;
...
{id:‘hospital’,label:‘病院’,svg:c=>`<svg width="16" height="16" viewBox="0 0 14 14" fill="none"><rect x="2" y="2" width="10" height="10" rx="1.5" stroke="${c}" stroke-width="1.3"/><line x1="7" y1="4.5" x2="7" y2="9.5" stroke="${c}" stroke-width="1.5" stroke-linecap="round"/><line x1="4.5" y1="7" x2="9.5" y2="7" stroke="${c}" stroke-width="1.5" stroke-linecap="round"/></svg>`},
{id:‘shop’,label:‘買い物’,svg:c=>`<svg width="16" height="16" viewBox="0 0 14 14" fill="none"><path d="M3 4.5h8l-1 7H4L3 4.5z" stroke="${c}" stroke-width="1.3" stroke-linejoin="round"/><path d="M5 4.5V3.5a2 2 0 014 0v1" stroke="${c}" stroke-width="1.3" stroke-linecap="round"/></svg>`},
{id:‘chores’,label:‘家事’,svg:c=>`<svg width="16" height="16" viewBox="0 0 14 14" fill="none"><path d="M7 2L2 6v6h4V9h2v3h4V6L7 2z" stroke="${c}" stroke-width="1.3" stroke-linejoin="round"/></svg>`},
...
{id:‘private’,label:‘プライベート’,svg:c=>`<svg width="16" height="16" viewBox="0 0 14 14" fill="none"><path d="M7 1.5l1.5 3.2 3.5.5-2.5 2.5.6 3.5L7 9.5l-3.1 1.7.6-3.5L2 5.2l3.5-.5L7 1.5z" fill="${c}"/></svg>`},
{id:‘other’,label:‘その他’,svg:c=>`<svg width="16" height="16" viewBox="0 0 14 14" fill="none"><circle cx="7" cy="7" r="5.5" stroke="${c}" stroke-width="1.3"/><circle cx="5" cy="7" r="0.9" fill="${c}"/><circle cx="7" cy="7" r="0.9" fill="${c}"/><circle cx="9" cy="7" r="0.9" fill="${c}"/></svg>`},
];
...
{id:3,text:‘写真撮影’,mins:60,color:’#F06E8A’,icon:‘active’,repeat:‘none’,subtasks:[],memo:’’,startH:null,startMin:null},
{id:4,text:‘Apple Watch動画’,mins:30,color:’#F4845A’,icon:‘private’,repeat:‘none’,subtasks:[],memo:’’,startH:14,startMin:30},
{id:5,text:‘領収書発行の準備’,mins:45,color:’#A07BC8’,icon:‘work’,repeat:‘none’,subtasks:[],memo:’’,startH:null,startMin:null},
];
...
<div id="hint-bar" style="position:absolute;bottom:0;left:0;right:0;background:rgba(240,110,138,.08);border-top:0.5px solid var(--pink-bg);padding:5px 12px;display:flex;gap:12px;align-items:center;z-index:100;flex-shrink:0;">
  <span style="font-size:9px;color:var(--muted);">🔵 長押し→移動</span>
  <span style="font-size:9px;color:var(--muted);">↕ 長押し→時間調整</span>
  <span style="font-size:9px;color:var(--muted);">📝 タップ→編集</span>
  <button onclick="document.getElementById('hint-bar').style.display='none'" style="margin-left:auto;border:none;background:transparent;font-size:11px;color:var(--muted);cursor:pointer;padding:2px;">✕</button>
</div>
...
<div id="tooltip" style="position:fixed;pointer-events:none;z-index:9998;display:none;background:rgba(26,26,26,.9);color:white;font-size:13px;font-weight:700;padding:6px 14px;border-radius:10px;white-space:nowrap;transform:translate(-50%,-150%);"></div>
...
const sh=document.getElementById(‘edit-start-h’),sm=document.getElementById(‘edit-start-m’);
sh.innerHTML=’’;sm.innerHTML=’’;
for(let h=0;h<24;h++){const o=document.createElement(‘option’);o.value=h;o.textContent=pad(h);sh.appendChild(o);}
for(let m=0;m<60;m+=5){const o=document.createElement(‘option’);o.value=m;o.textContent=pad(m);sm.appendChild(o);}
}
function onNoTimeChange(){
const noTime=document.getElementById(‘edit-no-time’).checked;
[‘edit-start-h’,‘edit-start-m’].forEach(id=>{const el=document.getElementById(id);el.disabled=noTime;el.style.opacity=noTime?‘0.35’:‘1’;});
}

// =========== TIMELINE ===========
function buildPillPath(x,y,w,h,ct,cb){
const r=w/2,cx=x+r;let d=’’;
if(ct){d+=`M ${x} ${y} A ${r} ${r} 0 0 0 ${x+w} ${y} `;if(cb)d+=`L ${x+w} ${y+h} A ${r} ${r} 0 0 0 ${x} ${y+h} Z`;else d+=`L ${x+w} ${y+h-r} A ${r} ${r} 0 0 1 ${cx} ${y+h} A ${r} ${r} 0 0 1 ${x} ${y+h-r} Z`;}
else{d+=`M ${x} ${y+r} A ${r} ${r} 0 0 1 ${cx} ${y} A ${r} ${r} 0 0 1 ${x+w} ${y+r} `;if(cb)d+=`L ${x+w} ${y+h} A ${r} ${r} 0 0 0 ${x} ${y+h} Z`;else d+=`L ${x+w} ${y+h-r} A ${r} ${r} 0 0 1 ${cx} ${y+h} A ${r} ${r} 0 0 1 ${x} ${y+h-r} Z`;}
return d;
}
...
for(let h=0;h<=24;h++){
const top=topPx(h,0);
const lbl=document.createElement(‘div’);lbl.style.cssText=‘position:absolute;left:0;top:’+(top-7)+‘px;width:’+LABEL_W+‘px;font-size:10px;color:var(–muted);font-weight:500;text-align:right;z-index:4;pointer-events:none;’;lbl.textContent=pad(h)+’:00’;wrap.appendChild(lbl);
if(h<24){const tick=document.createElement(‘div’);tick.style.cssText=‘position:absolute;left:’+TEXT_LEFT+‘px;right:0;top:’+top+‘px;height:0.5px;background:var(–line);z-index:1;pointer-events:none;’;wrap.appendChild(tick);}
}
...
// =========== ドラッグ状態 ===========
// type: ‘move’ | ‘resizeTop’ | ‘resizeBot’ | ‘schedule’
// itemId, startY, startMin / startTotal+startMins / startMins

function startLongPressMove(clientY, item){
// 長押しでバー移動開始
drag={type:‘move’,itemId:item.id,startY:clientY,startMin:item.startH*60+item.startMin};
addBarDragListeners();
}
function startLongPressResizeTop(clientY, item){
drag={type:‘resizeTop’,itemId:item.id,startY:clientY,startTotal:item.startH*60+item.startMin,startMins:item.mins};
addBarDragListeners();
}
function startLongPressResizeBot(clientY, item){
drag={type:‘resizeBot’,itemId:item.id,startY:clientY,startMins:item.mins};
addBarDragListeners();
}
function addBarDragListeners(){
document.addEventListener(‘mousemove’,onBarDragMove);document.addEventListener(‘mouseup’,onBarDragUp);
document.addEventListener(‘touchmove’,onBarDragTouch,{passive:false});document.addEventListener(‘touchend’,onBarDragUp);
}
function onBarDragMove(e){doBarDrag(e.clientX,e.clientY);}
function onBarDragTouch(e){e.preventDefault();doBarDrag(e.touches[0].clientX,e.touches[0].clientY);}
function doBarDrag(x,y){
if(!drag||drag.type===‘schedule’)return;
const allItems=currentItems();
const item=allItems.find(i=>i.id===drag.itemId||(i.isRepeat&&‘r’+i.templateId===String(drag.itemId)));
if(!item)return;
const mi=item.isRepeat?materialiseRepeat(item):findManualItem(item.id);if(!mi)return;
if(drag.type===‘move’){
const dm=Math.round((y-drag.startY)/PX/5)*5;
const nm=Math.max(0,Math.min(24*60-mi.mins,drag.startMin+dm));
mi.startH=Math.floor(nm/60);mi.startMin=nm%60;showTip(x,y,mi.startH+’:’+pad(mi.startMin));
} else if(drag.type===‘resizeTop’){
const dm=Math.round((y-drag.startY)/PX/5)*5;
const ns=Math.max(0,Math.min(drag.startTotal+drag.startMins-5,drag.startTotal+dm));
mi.startH=Math.floor(ns/60);mi.startMin=ns%60;mi.mins=Math.max(5,drag.startMins-(ns-drag.startTotal));
showTip(x,y,mi.startH+’:’+pad(mi.startMin));
} else if(drag.type===‘resizeBot’){
const dm=Math.round((y-drag.startY)/PX/5)*5;
const mx=24*60-(mi.startH*60+mi.startMin);
mi.mins=Math.max(5,Math.min(mx,drag.startMins+dm));showTip(x,y,endT(mi.startH,mi.startMin,mi.mins));
}
redraw();
}
function onBarDragUp(){
drag=null;hideTip();save();
document.removeEventListener(‘mousemove’,onBarDragMove);document.removeEventListener(‘mouseup’,onBarDragUp);
document.removeEventListener(‘touchmove’,onBarDragTouch);document.removeEventListener(‘touchend’,onBarDragUp);
}

// =========== RENDER BARS ===========
function renderBars(){
const svg=document.getElementById(‘bar-svg’);if(!svg)return;
const iov=document.getElementById(‘bar-iov’);if(!iov)return;
svg.innerHTML=’’;iov.innerHTML=’’;
...
items.forEach((item)=>{
const y=topPx(item.startH,item.startMin),h=Math.max(item.mins*PX,36),a=item.done?‘88’:’’;
const path=document.createElementNS(‘http://www.w3.org/2000/svg’,‘path’);
path.setAttribute(‘d’,buildPillPath(0,y,BAR_W,h,hasOvTop(items.indexOf(item)),hasOvBot(items.indexOf(item))));
path.setAttribute(‘fill’,item.color+a);svg.appendChild(path);

```
// アイコン領域（中央）← 長押しで移動
const iconAreaH=Math.min(h-40,40),iconY=y+h/2-iconAreaH/2;
const iconSize=20;
// アイコン描画
const fo=document.createElementNS('http://www.w3.org/2000/svg','foreignObject');
fo.setAttribute('x',(BAR_W-iconSize)/2);fo.setAttribute('y',y+h/2-iconSize/2);
fo.setAttribute('width',iconSize);fo.setAttribute('height',iconSize);
fo.style.cssText='pointer-events:none;';
const iconDiv=document.createElement('div');
iconDiv.style.cssText='width:'+iconSize+'px;height:'+iconSize+'px;border-radius:5px;background:rgba(255,255,255,.3);display:flex;align-items:center;justify-content:center;';
iconDiv.innerHTML=iconSvg(item.icon,'white');
fo.appendChild(iconDiv);svg.appendChild(fo);

// ---- バー操作エリア (HTML overlay) ----
// 上ハンドル（長押し → リサイズTop）
const HANDLE_H=28;
const topHandle=document.createElement('div');
topHandle.style.cssText=`position:absolute;left:0;top:${y}px;width:${BAR_W}px;height:${HANDLE_H}px;
  display:flex;align-items:flex-start;justify-content:center;padding-top:5px;
  cursor:n-resize;pointer-events:all;z-index:10;`;
topHandle.innerHTML=`<div style="width:22px;height:5px;background:rgba(255,255,255,.85);border-radius:4px;box-shadow:0 1px 4px rgba(0,0,0,.2);"></div>`;

setupLongPress(topHandle, item,
  (clientY)=>startLongPressResizeTop(clientY, item),
  null // タップは何もしない
);
iov.appendChild(topHandle);

// 下ハンドル（長押し → リサイズBot）
const botHandle=document.createElement('div');
botHandle.style.cssText=`position:absolute;left:0;top:${y+h-HANDLE_H}px;width:${BAR_W}px;height:${HANDLE_H}px;
  display:flex;align-items:flex-end;justify-content:center;padding-bottom:5px;
  cursor:s-resize;pointer-events:all;z-index:10;`;
botHandle.innerHTML=`<div style="width:22px;height:5px;background:rgba(255,255,255,.85);border-radius:4px;box-shadow:0 1px 4px rgba(0,0,0,.2);"></div>`;
setupLongPress(botHandle, item,
  (clientY)=>startLongPressResizeBot(clientY, item),
  null
);
iov.appendChild(botHandle);

// 中央エリア（長押し → 移動）
const midTop=y+HANDLE_H,midH=Math.max(h-HANDLE_H*2,8);
const midArea=document.createElement('div');
midArea.style.cssText=`position:absolute;left:0;top:${midTop}px;width:${BAR_W}px;height:${midH}px;
  cursor:grab;pointer-events:all;z-index:9;`;
setupLongPress(midArea, item,
  (clientY)=>startLongPressMove(clientY, item),
  null
);
iov.appendChild(midArea);
```
...
if(sameDay(currentDate,td)){
const dot=document.createElementNS(‘http://www.w3.org/2000/svg’,‘circle’);
dot.setAttribute(‘cx’,R);dot.setAttribute(‘cy’,topPx(n.getHours(),n.getMinutes()));
dot.setAttribute(‘r’,5);dot.setAttribute(‘fill’,‘var(–pink)’);dot.setAttribute(‘stroke’,‘white’);dot.setAttribute(‘stroke-width’,‘2’);
dot.style.pointerEvents=‘none’;svg.appendChild(dot);
}
}

// 長押し処理ヘルパー
function setupLongPress(el, item, onLongPress, onTap){
let timer=null,fired=false,sx=0,sy=0;
el.addEventListener(‘mousedown’,e=>{
e.stopPropagation();e.preventDefault();
fired=false;sx=e.clientX;sy=e.clientY;
timer=setTimeout(()=>{fired=true;el.style.opacity=‘0.7’;onLongPress&&onLongPress(e.clientY);},LONG_PRESS_MS);
const up=()=>{clearTimeout(timer);el.style.opacity=’’;if(!fired&&onTap)onTap();document.removeEventListener(‘mouseup’,up);};
document.addEventListener(‘mouseup’,up);
});
el.addEventListener(‘touchstart’,e=>{
e.stopPropagation();
fired=false;sx=e.touches[0].clientX;sy=e.touches[0].clientY;
timer=setTimeout(()=>{fired=true;el.style.opacity=‘0.7’;onLongPress&&onLongPress(e.touches[0].clientY);},LONG_PRESS_MS);
},{passive:true});
el.addEventListener(‘touchmove’,e=>{
if(!fired){const dx=e.touches[0].clientX-sx,dy=e.touches[0].clientY-sy;if(Math.sqrt(dx*dx+dy*dy)>6)clearTimeout(timer);}
},{passive:true});
el.addEventListener(‘touchend’,e=>{
clearTimeout(timer);el.style.opacity=’’;
...
const n=new Date(),td=today0();
if(sameDay(currentDate,td)){const nl=document.createElement(‘div’);nl.style.cssText=‘position:absolute;left:0;right:0;top:’+topPx(n.getHours(),n.getMinutes())+‘px;height:1px;background:var(–pink);opacity:.3;z-index:1;pointer-events:none;’;layer.appendChild(nl);}

items.forEach((item,idx)=>{
const barTop=topPx(item.startH,item.startMin),barH=Math.max(item.mins*PX,36);
const et=endT(item.startH,item.startMin,item.mins),tr=item.startH+’:’+pad(item.startMin)+’〜’+et;
const isShort=barH<44,showBadge=item.isRepeat||item.hasRepeatBadge;
const rb=showBadge?’<span style="font-size:11px;margin-left:2px;">🔁</span>’:’’;
const hasSub=(item.subtasks||[]).length>0,hasMemo=item.memo&&item.memo.trim();

```
const row=document.createElement('div');
row.style.cssText=`position:absolute;left:0;right:0;top:${barTop}px;height:${barH}px;
  display:flex;align-items:center;gap:4px;padding:0 5px 0 6px;overflow:visible;`;

// タスク名部分 → タップで編集モーダルを開く
const txt=document.createElement('div');txt.style.cssText='flex:1;min-width:0;cursor:pointer;user-select:none;-webkit-user-select:none;';
if(isShort){
  txt.innerHTML=`<div style="display:flex;align-items:center;gap:3px;white-space:nowrap;">
    <span style="font-size:11px;font-weight:600;color:${item.done?'var(--muted)':'var(--text)'};${item.done?'text-decoration:line-through;':''};overflow:hidden;text-overflow:ellipsis;max-width:90px;">${esc(item.text)}</span>${rb}
    <span style="font-size:10px;color:${item.color};flex-shrink:0;">${tr}</span></div>`;
} else {
  txt.innerHTML=`<div style="font-size:10px;color:${item.color};white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${tr}${rb}</div>
    <div style="font-size:12px;font-weight:600;color:${item.done?'var(--muted)':'var(--text)'};line-height:1.3;${item.done?'text-decoration:line-through;':''};white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${esc(item.text)}</div>
    ${hasSub||hasMemo?`<div style="display:flex;gap:4px;margin-top:1px;">${hasSub?`<span style="font-size:9px;color:var(--muted);">📋${item.subtasks.length}</span>`:''}${hasMemo?'<span style="font-size:9px;color:var(--muted);">📝</span>':''}</div>`:''}`;
}
// タップ → 編集モーダル
txt.addEventListener('click',()=>openEditModal('sched',null,idx));

// チェック・戻すボタン
const acts=document.createElement('div');acts.style.cssText='display:flex;gap:3px;align-items:center;flex-shrink:0;';
const chk=document.createElement('div');chk.style.cssText=`width:22px;height:22px;border-radius:50%;border:1.5px solid ${item.done?item.color:'#D0C8C0'};background:${item.done?item.color:'transparent'};display:flex;align-items:center;justify-content:center;cursor:pointer;`;
if(item.done)chk.innerHTML='<svg width="10" height="10" viewBox="0 0 10 10" fill="none"><path d="M1.5 5L4 7.5L8.5 2.5" stroke="white" stroke-width="1.6" stroke-linecap="round"/></svg>';
...
if(!item.isRepeat){
  const back=document.createElement('div');back.style.cssText='width:22px;height:22px;border-radius:50%;background:var(--pink-bg);display:flex;align-items:center;justify-content:center;cursor:pointer;';
  back.innerHTML='<svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M10 6A4 4 0 106 2.2" stroke="var(--pink)" stroke-width="1.4" stroke-linecap="round"/><path d="M6 2l-.2 2.5L8 5" stroke="var(--pink)" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/></svg>';
  back.addEventListener('click',e=>{e.stopPropagation();returnToList(idx);});
  acts.appendChild(chk);acts.appendChild(back);
} else {acts.appendChild(chk);}
...
const useH=(t.startH!==null&&t.startH!==undefined)?t.startH:h;
const useM=(t.startMin!==null&&t.startMin!==undefined)?t.startMin:0;
schedByDate[key].push({id:nextId++,text:t.text,mins:t.mins,color:t.color,icon:t.icon||‘other’,done:false,startH:useH,startMin:useM,repeat:‘none’,subtasks:JSON.parse(JSON.stringify(t.subtasks||[])),memo:t.memo||’’});
tasks=tasks.filter(t=>t.id!==drag.taskId);drag=null;
save();renderTasks();redraw();renderWeekStrip();
document.getElementById(‘tl-scroll’).scrollTo({top:Math.max(0,topPx(useH,useM)-80),behavior:‘smooth’});
}
...
editTarget={type:‘new’};document.getElementById(‘modal-title’).textContent=‘タスクを追加’;
document.getElementById(‘edit-name’).value=document.getElementById(‘inp-name’).value.trim();
document.getElementById(‘edit-mins’).value=document.getElementById(‘inp-mins’).value;
document.getElementById(‘edit-repeat’).value=‘none’;onRepeatChange();
selectedColor=PALETTE[0];selectedIcon=‘work’;modalSubtasks=[];
renderColorPicker();renderIconPicker();renderModalSubtasks();
document.getElementById(‘edit-memo’).value=’’;populateDayPicker([]);populateTimePickers();
const now=new Date();
document.getElementById(‘edit-start-h’).value=now.getHours();
document.getElementById(‘edit-start-m’).value=Math.floor(now.getMinutes()/5)*5;
document.getElementById(‘edit-no-time’).checked=true;onNoTimeChange();
document.getElementById(‘edit-modal’).style.display=‘flex’;
}
function openEditModal(type,id,idx){
let item;
if(type===‘task’)item=tasks.find(t=>t.id===id);
else if(type===‘repeat’)item=repeatTemplates.find(t=>t.id===id);
else{const arr=currentItems();item=arr[idx];}
if(!item)return;
...
if(item.startH!==undefined&&item.startH!==null){document.getElementById(‘repeat-start-h’).value=item.startH;document.getElementById(‘repeat-start-m’).value=item.startMin||0;}
const hasStart=item.startH!==null&&item.startH!==undefined;
document.getElementById(‘edit-no-time’).checked=!hasStart;
document.getElementById(‘edit-start-h’).value=hasStart?item.startH:new Date().getHours();
document.getElementById(‘edit-start-m’).value=hasStart?(item.startMin||0):0;
onNoTimeChange();
document.getElementById(‘edit-modal’).style.display=‘flex’;
}
...
const rStartH=parseInt(document.getElementById(‘repeat-start-h’).value)||0,rStartM=parseInt(document.getElementById(‘repeat-start-m’).value)||0;
...
const noTime=document.getElementById(‘edit-no-time’).checked;
const startH=noTime?null:parseInt(document.getElementById(‘edit-start-h’).value);
const startMin=noTime?null:parseInt(document.getElementById(‘edit-start-m’).value)||0;

if(editTarget.type===‘new’){
if(repeat!==‘none’)repeatTemplates.push({id:nextId++,text:name,mins,color,icon,repeat,repeatDays,startH:rStartH,startMin:rStartM,subtasks,memo,repeatFromDate:currentKey()});
else{tasks.push({id:nextId++,text:name,mins,color,icon,repeat:‘none’,subtasks,memo,startH,startMin});document.getElementById(‘inp-name’).value=’’;}
} else if(editTarget.type===‘task’){
const t=tasks.find(t=>t.id===editTarget.id);if(t){t.text=name;t.mins=mins;t.color=color;t.icon=icon;t.subtasks=subtasks;t.memo=memo;t.startH=startH;t.startMin=startMin;}
if(repeat!==‘none’){repeatTemplates.push({id:nextId++,text:name,mins,color,icon,repeat,repeatDays,startH:rStartH,startMin:rStartM,subtasks,memo,repeatFromDate:currentKey()});tasks=tasks.filter(t=>t.id!==editTarget.id);}
} else if(editTarget.type===‘repeat’){
const tmpl=repeatTemplates.find(t=>t.id===editTarget.id);if(tmpl){tmpl.text=name;tmpl.mins=mins;tmpl.color=color;tmpl.icon=icon;tmpl.subtasks=subtasks;tmpl.memo=memo;tmpl.repeat=repeat;tmpl.repeatDays=repeatDays;tmpl.startH=rStartH;tmpl.startMin=rStartM;}
} else if(editTarget.type===‘sched’){
const items=currentItems();const srcItem=items[editTarget.idx];
if(srcItem&&!srcItem.isRepeat){const mItem=findManualItem(srcItem.id);if(mItem){mItem.text=name;mItem.mins=mins;mItem.color=color;mItem.icon=icon;mItem.subtasks=subtasks;mItem.memo=memo;if(!noTime){mItem.startH=startH;mItem.startMin=startMin;}if(repeat!==‘none’)mItem.hasRepeatBadge=true;}}
...
if(repeat!==‘none’){const tomorrow=new Date(currentDate.getTime()+86400000);const src=items[editTarget.idx];repeatTemplates.push({id:nextId++,text:name,mins,color,icon,repeat,repeatDays,startH:src?src.startH:rStartH,startMin:src?src.startMin:rStartM,subtasks,memo,repeatFromDate:dateKey(tomorrow)});}
}
