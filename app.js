// FocusFlow app.js v22 - 開始時刻設定 + ドラッグ改善
'use strict';
...
<div style="background:var(--white);border-radius:18px;padding:18px;width:100%;max-width:310px;margin:0 auto;">
  ...
  <div style="display:flex;gap:8px;align-items:center;margin-bottom:12px;">
    ...
    <label style="display:flex;align-items:center;gap:5px;font-size:11px;color:var(--muted);white-space:nowrap;cursor:pointer;">
      <input type="checkbox" id="edit-no-time" onchange="onNoTimeChange()" style="accent-color:var(--pink);width:15px;height:15px;"/>未設定
    </label>
  </div>
...
<div style="background:var(--white);border-radius:18px;padding:20px;width:260px;">
  ...
  <div style="display:flex;flex-direction:column;gap:9px;">
    ...
    <button onclick="deleteAllRepeat()" style="padding:11px;border-radius:11px;border:none;background:#FEE;font-size:12px;color:#E05050;cursor:pointer;text-align:left;"><div style="font-weight:600;margin-bottom:2px;">すべての繰り返しを削除</div><div style="font-size:11px;color:#E07070;">繰り返し設定ごと削除します</div></button>
...
function renderBars(){
  ...
  items.forEach((item,idx)=>{
    ...
    rect.setAttribute('x',0);rect.setAttribute('y',y+18);rect.setAttribute('width',BAR_W);rect.setAttribute('height',Math.max(h-36,4));
    rect.setAttribute('fill','transparent');rect.style.cssText='cursor:grab;';
    ...
    th.style.cssText='position:absolute;left:-6px;top:'+(y-4)+'px;width:'+(BAR_W+12)+'px;height:24px;cursor:n-resize;display:flex;align-items:center;justify-content:center;pointer-events:all;z-index:10;';
    ...
    th.addEventListener('touchstart',e=>{e.stopPropagation();e.preventDefault();startResizeTop(e.touches[0].clientY,item.id,item.startH,item.startMin,item.mins);},{passive:false});
    iov.appendChild(th);

    // ★ 下ハンドル（高さ24px、幅広め）
    const bh=document.createElement('div');
    bh.style.cssText='position:absolute;left:-6px;top:'+(y+h-20)+'px;width:'+(BAR_W+12)+'px;height:24px;cursor:s-resize;display:flex;align-items:center;justify-content:center;pointer-events:all;z-index:10;';
    ...
    bh.addEventListener('touchstart',e=>{e.stopPropagation();e.preventDefault();startResizeBot(e.touches[0].clientY,item.id,item.mins);},{passive:false});
    iov.appendChild(bh);
  });
...
function renderTexts(){
  ...
  items.forEach((item,idx)=>{
    ...
    row.style.cssText='position:absolute;left:0;right:0;top:'+barTop+'px;height:'+barH+'px;display:flex;align-items:center;gap:3px;padding:0 4px 0 8px;overflow:visible;';
    const txt=document.createElement('div');txt.style.cssText='flex:1;min-width:0;user-select:none;';
    ...
    // ★ 長押し(280ms)でドラッグ、短タップで詳細
    let lpTimer=null,lpFired=false,startX=0,startY2=0;
    txt.addEventListener('touchstart',e=>{
      lpFired=false;startX=e.touches[0].clientX;startY2=e.touches[0].clientY;
      lpTimer=setTimeout(()=>{lpFired=true;txt.style.transform='scale(1.03)';txt.style.transition='transform .1s';startMove(startY2,item.id,item.startH,item.startMin);},LONG_PRESS_MS);
    },{passive:true});
    txt.addEventListener('touchmove',e=>{
      const dx=e.touches[0].clientX-startX,dy=e.touches[0].clientY-startY2;
      if(!lpFired&&Math.sqrt(dx*dx+dy*dy)>6){clearTimeout(lpTimer);}
    },{passive:true});
    txt.addEventListener('touchend',e=>{
      clearTimeout(lpTimer);txt.style.transform='';txt.style.transition='';
      if(!lpFired)openDetail(idx);
    },{passive:true});
    ...
    if(!item.isRepeat){
      const back=document.createElement('div');back.style.cssText='width:20px;height:20px;border-radius:50%;background:var(--pink-bg);display:flex;align-items:center;justify-content:center;cursor:pointer;';
      back.innerHTML='<svg width="11" height="11" viewBox="0 0 11 11" fill="none"><path d="M9 5A4 4 0 105.3 1.3" stroke="var(--pink)" stroke-width="1.3" stroke-linecap="round"/><path d="M5 1l.5 2.5L3 4" stroke="var(--pink)" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"/></svg>';
...
function openAddModal(){
  ...
  populateDayPicker([]);populateTimePickers();
  const now=new Date();
