...
function save(){
  try{
    ...
    localStorage.setItem('ff_sched',JSON.stringify(schedByDate));
    localStorage.setItem('ff_repeat',JSON.stringify(repeatTemplates));
    localStorage.setItem('ff_nextId',String(nextId));
  }catch(e){console.warn('save failed',e);}
}
function load(){
  try{
    const t=localStorage.getItem('ff_tasks');if(t)tasks=JSON.parse(t);
    const s=localStorage.getItem('ff_sched');if(s)schedByDate=JSON.parse(s);
    const r=localStorage.getItem('ff_repeat');if(r)repeatTemplates=JSON.parse(r);
    const n=localStorage.getItem('ff_nextId');if(n)nextId=parseInt(n)||10;
  }catch(e){console.warn('load failed',e);}
}

// ---- State ----
let tasks=[
  {id:1,text:'明日のMTG準備',mins:15,color:'#F06E8A',icon:'work',repeat:'none',subtasks:[],memo:''},
  {id:2,text:'店舗営業日カレンダー',mins:30,color:'#5B9BD5',icon:'work',repeat:'none',subtasks:[],memo:''},
  {id:3,text:'写真撮影',mins:15,color:'#F06E8A',icon:'active',repeat:'none',subtasks:[],memo:''},
  {id:4,text:'Apple Watch動画',mins:30,color:'#F4845A',icon:'private',repeat:'none',subtasks:[],memo:''},
  {id:5,text:'領収書発行の準備',mins:15,color:'#A07BC8',icon:'work',repeat:'none',subtasks:[],memo:''},
];
let schedByDate={},repeatTemplates=[];
let nextId=10;

let weekStart=getWeekStart(new Date()),currentDate=new Date();currentDate.setHours(0,0,0,0);
...
function matchesRepeat(tmpl,d){
  ...
  const from=dateFromKey(tmpl.repeatFromDate);
  if(d<from)return false;
  if((tmpl.skipDates||[]).includes(dateKey(d)))return false;
  const dow=d.getDay();
  if(tmpl.repeat==='daily')return true;
  if(tmpl.repeat==='weekday')return dow>=1&&dow<=5;
  if(tmpl.repeat==='weekend')return dow===0||dow===6;
  if(tmpl.repeat==='weekly')return(tmpl.repeatDays||[]).includes(dow);
  return false;
}
...
function findManualItem(itemId){for(const[,arr]of Object.entries(schedByDate)){const f=arr.find(i=>i.id===itemId);if(f)return f;}return null;}
...
<div style="background:var(--white);border-bottom:0.5px solid var(--line);flex-shrink:0;">
  <div style="display:flex;align-items:center;justify-content:space-between;padding:9px 16px 5px;">
    <div style="display:flex;align-items:center;gap:6px;">
      <div style="width:22px;height:22px;background:var(--pink);border-radius:6px;display:flex;align-items:center;justify-content:center;">
        <svg width="11" height="11" viewBox="0 0 11 11" fill="none"><circle cx="5.5" cy="5.5" r="4.5" stroke="white" stroke-width="1.2"/><line x1="5.5" y1="3" x2="5.5" y2="5.5" stroke="white" stroke-width="1.2" stroke-linecap="round"/><line x1="5.5" y1="5.5" x2="7.5" y2="6.8" stroke="white" stroke-width="1.2" stroke-linecap="round"/></svg>
      </div>
      <div id="ym-label" style="font-size:14px;font-weight:700;color:var(--pink);"></div>
    </div>
    <div style="display:flex;align-items:center;gap:4px;">
      <button onclick="changeWeek(-1)" style="width:28px;height:28px;border:none;background:var(--pink-bg);border-radius:7px;cursor:pointer;display:flex;align-items:center;justify-content:center;padding:0;"><svg width="7" height="11" viewBox="0 0 7 11" fill="none"><path d="M5 1.5L2 5.5L5 9.5" stroke="var(--pink)" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/></svg></button>
...
<div style="display:flex;flex:1;overflow:hidden;min-height:0;">
  <div style="width:128px;min-width:128px;background:var(--white);border-right:0.5px solid var(--line);display:flex;flex-direction:column;overflow:hidden;">
    ...
    <div id="panel-task" style="flex:1;display:flex;flex-direction:column;overflow:hidden;">
      ...
      <div style="padding:6px 8px;border-top:0.5px solid var(--line);flex-shrink:0;">
        <input id="inp-name" type="text" placeholder="タスク名..." style="width:100%;padding:5px 6px;border-radius:7px;border:0.5px solid var(--line);background:var(--pink-bg);color:var(--text);outline:none;font-size:11px;margin-bottom:4px;" onfocus="this.style.borderColor='var(--pink)'" onblur="this.style.borderColor='var(--line)'"/>
        <div style="display:flex;gap:4px;">
          <select id="inp-mins" style="flex:1;font-size:11px;background:var(--pink-bg);border:0.5px solid var(--line);border-radius:6px;color:var(--text);outline:none;padding:4px 3px;">
          ...
          <button onclick="openAddModal()" style="background:var(--pink);border:none;border-radius:6px;width:26px;height:26px;display:flex;align-items:center;justify-content:center;padding:0;flex-shrink:0;"><svg width="10" height="10" viewBox="0 0 10 10" fill="none"><line x1="5" y1="1" x2="5" y2="9" stroke="white" stroke-width="1.5" stroke-linecap="round"/><line x1="1" y1="5" x2="9" y2="5" stroke="white" stroke-width="1.5" stroke-linecap="round"/></svg></button>
        </div>
      </div>
    </div>
  ...
  <div style="flex:1;display:flex;flex-direction:column;overflow:hidden;">
    ...
    <div id="tl-scroll" style="flex:1;overflow-y:auto;padding:0 12px 60px;">
      <div id="tl-wrap" style="position:relative;"></div>
    </div>
  </div>
</div>
<div id="ghost" style="position:fixed;pointer-events:none;z-index:9999;display:none;background:var(--pink);color:white;font-size:11px;font-weight:500;padding:5px 12px;border-radius:18px;white-space:nowrap;transform:translate(-50%,-50%);box-shadow:0 4px 12px rgba(0,0,0,.2);"></div>
<div id="tooltip" style="position:fixed;pointer-events:none;z-index:9998;display:none;background:rgba(26,26,26,0.85);color:white;font-size:11px;padding:4px 10px;border-radius:8px;white-space:nowrap;transform:translate(-50%,-130%);"></div>
<!-- Edit Modal -->
<div id="edit-modal" style="display:none;position:absolute;inset:0;z-index:8000;background:rgba(0,0,0,0.28);align-items:flex-start;justify-content:center;overflow-y:auto;padding:20px 12px;">
  <div style="background:var(--white);border-radius:18px;padding:18px;width:100%;max-width:320px;margin:0 auto;">
    ...
    <input id="edit-name" type="text" style="width:100%;padding:8px 10px;border-radius:9px;border:0.5px solid var(--line);background:var(--pink-bg);color:var(--text);outline:none;font-size:13px;margin-bottom:12px;" onfocus="this.style.borderColor='var(--pink)'" onblur="this.style.borderColor='var(--line)'"/>
    <div style="font-size:11px;color:var(--muted);margin-bottom:4px;">所要時間</div>
    <select id="edit-mins" style="width:100%;padding:8px 10px;font-size:13px;border-radius:9px;border:0.5px solid var(--line);background:var(--pink-bg);color:var(--text);outline:none;margin-bottom:12px;">
    ...
    <div id="color-picker" style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:12px;"></div>
    ...
    <select id="edit-repeat" onchange="onRepeatChange()" style="width:100%;padding:8px 10px;font-size:13px;border-radius:9px;border:0.5px solid var(--line);background:var(--pink-bg);color:var(--text);outline:none;margin-bottom:10px;">
    ...
    <div id="repeat-time-row" style="display:none;margin-bottom:12px;">
      ...
      <div style="display:flex;gap:8px;align-items:center;">
        <select id="repeat-start-h" style="flex:1;padding:6px;font-size:13px;border-radius:9px;border:0.5px solid var(--line);background:var(--pink-bg);color:var(--text);outline:none;"></select>
        <span style="color:var(--muted);font-size:14px;">:</span>
        <select id="repeat-start-m" style="flex:1;padding:6px;font-size:13px;border-radius:9px;border:0.5px solid var(--line);background:var(--pink-bg);color:var(--text);outline:none;"></select>
      </div>
    </div>
    ...
    <div style="display:flex;gap:5px;margin-bottom:12px;">
      <input id="subtask-inp" type="text" placeholder="サブタスクを追加..." style="flex:1;padding:6px 9px;border-radius:8px;border:0.5px solid var(--line);background:var(--pink-bg);color:var(--text);outline:none;font-size:12px;" onfocus="this.style.borderColor='var(--pink)'" onblur="this.style.borderColor='var(--line)'"/>
      <button onclick="addSubtask()" style="background:var(--pink);border:none;border-radius:8px;width:28px;height:30px;display:flex;align-items:center;justify-content:center;padding:0;cursor:pointer;flex-shrink:0;"><svg width="10" height="10" viewBox="0 0 10 10" fill="none"><line x1="5" y1="1" x2="5" y2="9" stroke="white" stroke-width="1.5" stroke-linecap="round"/><line x1="1" y1="5" x2="9" y2="5" stroke="white" stroke-width="1.5" stroke-linecap="round"/></svg></button>
    </div>
    <div style="font-size:11px;color:var(--muted);margin-bottom:4px;">メモ</div>
    <textarea id="edit-memo" rows="3" placeholder="メモを入力..." style="width:100%;padding:8px 10px;border-radius:9px;border:0.5px solid var(--line);background:var(--pink-bg);color:var(--text);outline:none;font-size:12px;margin-bottom:16px;" onfocus="this.style.borderColor='var(--pink)'" onblur="this.style.borderColor='var(--line)'"></textarea>
...
<!-- Detail panel -->
<div id="detail-panel" style="display:none;position:absolute;inset:0;z-index:7000;background:rgba(0,0,0,0.22);align-items:flex-end;justify-content:center;">
...
<!-- Delete (repeat) modal -->
<div id="delete-modal" style="display:none;position:absolute;inset:0;z-index:9000;background:rgba(0,0,0,0.32);align-items:center;justify-content:center;">
  <div style="background:var(--white);border-radius:18px;padding:20px;width:260px;">
    ...
    <div style="display:flex;flex-direction:column;gap:9px;">
      <button onclick="deleteOnlyToday()" style="padding:11px;border-radius:11px;border:0.5px solid var(--line);background:var(--white);font-size:12px;color:var(--text);cursor:pointer;text-align:left;">
        <div style="font-weight:600;margin-bottom:2px;">この日だけ削除</div>
        <div style="font-size:11px;color:var(--muted);">他の日の繰り返しはそのまま</div>
      </button>
      <button onclick="deleteAllRepeat()" style="padding:11px;border-radius:11px;border:none;background:#FEE;font-size:12px;color:#E05050;cursor:pointer;text-align:left;">
        <div style="font-weight:600;margin-bottom:2px;">すべての繰り返しを削除</div>
        <div style="font-size:11px;color:#E07070;">繰り返し設定ごと削除します</div>
      </button>
...
<div id="simple-delete-modal" style="display:none;position:absolute;inset:0;z-index:9000;background:rgba(0,0,0,0.32);align-items:center;justify-content:center;">
...
<!-- Rollover modal -->
<div id="rollover-modal" style="display:none;position:absolute;inset:0;z-index:8000;background:rgba(0,0,0,0.28);align-items:center;justify-content:center;">
...
function switchTab(tab){
  currentTab=tab;
  document.getElementById('panel-task').style.display=tab==='task'?'flex':'none';
  document.getElementById('panel-done').style.display=tab==='done'?'block':'none';
  ['task','done'].forEach(t=>{const b=document.getElementById('tab-'+t);b.style.background=t===tab?'var(--pink-bg)':'transparent';b.style.color=t===tab?'var(--pink)':'var(--muted)';b.style.fontWeight=t===tab?'600':'500';});
  if(tab==='done')renderDonePanel();
}
function renderDonePanel(){
  const panel=document.getElementById('panel-done');panel.innerHTML='';
  const done=[];Object.entries(schedByDate).forEach(([dk,arr])=>{arr.filter(i=>i.done).forEach(i=>done.push({...i,dateKey:dk}));});
  if(!done.length){panel.innerHTML='<div style="font-size:11px;color:var(--muted);text-align:center;padding:18px 0;">完了タスクなし</div>';return;}
  done.sort((a,b)=>(b.dateKey||'').localeCompare(a.dateKey||''));
  done.forEach(item=>{
    const el=document.createElement('div');el.style.cssText='background:var(--bg);border-radius:8px;padding:6px 7px;margin-bottom:5px;';
    el.innerHTML='<div style="display:flex;align-items:center;gap:5px;"><div style="width:18px;height:18px;border-radius:5px;background:'+item.color+'88;display:flex;align-items:center;justify-content:center;flex-shrink:0;">'+iconSvg(item.icon||'other','white')+'</div><div style="flex:1;min-width:0;"><div style="font-size:11px;color:var(--muted);text-decoration:line-through;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">'+esc(item.text)+'</div><div style="font-size:10px;color:var(--muted);">'+item.dateKey+'</div></div><div onclick="rolloverItem(\''+item.dateKey+'\','+item.id+')" style="font-size:10px;color:var(--pink);cursor:pointer;background:var(--pink-bg);padding:3px 6px;border-radius:5px;flex-shrink:0;">繰越</div></div>';
...
function renderModalSubtasks(){
  ...
  modalSubtasks.forEach((st,i)=>{
    ...
    const chk=document.createElement('div');chk.style.cssText='width:16px;height:16px;border-radius:5px;border:1.5px solid '+(st.done?selectedColor:'#D0C8C0')+';background:'+(st.done?selectedColor:'transparent')+';display:flex;align-items:center;justify-content:center;cursor:pointer;flex-shrink:0;';
    if(st.done)chk.innerHTML='<svg width="8" height="8" viewBox="0 0 8 8" fill="none"><path d="M1 4L3 6L7 2" stroke="white" stroke-width="1.4" stroke-linecap="round"/></svg>';
    chk.addEventListener('click',()=>{modalSubtasks[i].done=!modalSubtasks[i].done;renderModalSubtasks();});
    const lbl=document.createElement('div');lbl.style.cssText='flex:1;font-size:12px;color:'+(st.done?'var(--muted)':'var(--text)')+';'+(st.done?'text-decoration:line-through;':'');lbl.textContent=st.text;
    const del=document.createElement('div');del.style.cssText='width:16px;height:16px;border-radius:50%;background:rgba(0,0,0,0.06);display:flex;align-items:center;justify-content:center;cursor:pointer;flex-shrink:0;';del.innerHTML='<svg width="8" height="8" viewBox="0 0 8 8" fill="none"><line x1="2" y1="2" x2="6" y2="6" stroke="var(--muted)" stroke-width="1.3" stroke-linecap="round"/><line x1="6" y1="2" x2="2" y2="6" stroke="var(--muted)" stroke-width="1.3" stroke-linecap="round"/></svg>';
    del.addEventListener('click',()=>{modalSubtasks.splice(i,1);renderModalSubtasks();});
    row.appendChild(chk);row.appendChild(lbl);row.appendChild(del);list.appendChild(row);
  });
}
...
function buildTimeline(){
  ...
  for(let h=0;h<24;h++){const dz=document.createElement('div');dz.className='dz';dz.dataset.hour=h;dz.style.cssText='position:absolute;left:'+TEXT_LEFT+'px;right:0;top:'+topPx(h,0)+'px;height:'+HOUR_H+'px;z-index:2;border-radius:8px;transition:background 0.1s;';wrap.appendChild(dz);}
...
function renderBars(){
  ...
  items.forEach((item,idx)=>{
    ...
    if(h>=22){const fo=document.createElementNS('http://www.w3.org/2000/svg','foreignObject');const is=16;fo.setAttribute('x',(BAR_W-is)/2);fo.setAttribute('y',y+h/2-is/2);fo.setAttribute('width',is);fo.setAttribute('height',is);fo.style.cssText='pointer-events:none;';const div=document.createElement('div');div.style.cssText='width:'+is+'px;height:'+is+'px;border-radius:4px;background:rgba(255,255,255,0.28);display:flex;align-items:center;justify-content:center;';div.innerHTML=iconSvg(item.icon,'white');fo.appendChild(div);svg.appendChild(fo);}
    const rect=document.createElementNS('http://www.w3.org/2000/svg','rect');rect.setAttribute('x',0);rect.setAttribute('y',y+10);rect.setAttribute('width',BAR_W);rect.setAttribute('height',Math.max(h-20,2));rect.setAttribute('fill','transparent');rect.style.cssText='cursor:grab;';
    ...
    rect.addEventListener('touchstart',e=>{e.stopPropagation();e.preventDefault();startMove(e.touches[0].clientY,item.id,item.startH,item.startMin);},{passive:false});svg.appendChild(rect);
    const th=document.createElement('div');th.style.cssText='position:absolute;left:0;top:'+y+'px;width:'+BAR_W+'px;height:10px;cursor:n-resize;display:flex;align-items:center;justify-content:center;pointer-events:all;';th.innerHTML='<div style="width:12px;height:2px;background:rgba(255,255,255,0.5);border-radius:2px;"></div>';
    ...
    const bh=document.createElement('div');bh.style.cssText='position:absolute;left:0;top:'+(y+h-10)+'px;width:'+BAR_W+'px;height:10px;cursor:s-resize;display:flex;align-items:center;justify-content:center;pointer-events:all;';bh.innerHTML='<div style="width:12px;height:2px;background:rgba(255,255,255,0.5);border-radius:2px;"></div>';
...
function renderTexts(){
  ...
  const n=new Date(),td=today0();
  if(sameDay(currentDate,td)){const nl=document.createElement('div');nl.style.cssText='position:absolute;left:0;right:0;top:'+topPx(n.getHours(),n.getMinutes())+'px;height:1px;background:var(--pink);opacity:0.3;z-index:1;pointer-events:none;';layer.appendChild(nl);}
  items.forEach((item,idx)=>{
    ...
    const txt=document.createElement('div');txt.style.cssText='flex:1;min-width:0;cursor:pointer;';
    if(isShort){txt.innerHTML='<div style="display:flex;align-items:center;gap:3px;white-space:nowrap;"><span style="font-size:11px;font-weight:600;color:'+(item.done?'var(--muted)':'var(--text)')+';'+(item.done?'text-decoration:line-through;':'')+';overflow:hidden;text-overflow:ellipsis;max-width:80px;">'+esc(item.text)+'</span>'+rb+'<span style="font-size:10px;color:'+item.color+';flex-shrink:0;">'+tr+'</span></div>';}
    else{txt.innerHTML='<div style="font-size:10px;color:'+item.color+';white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">'+tr+rb+'</div><div style="font-size:12px;font-weight:600;color:'+(item.done?'var(--muted)':'var(--text)')+';line-height:1.3;'+(item.done?'text-decoration:line-through;':'')+';white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">'+esc(item.text)+'</div>'+(hasSub||hasMemo?'<div style="display:flex;gap:4px;margin-top:1px;">'+(hasSub?'<span style="font-size:9px;color:var(--muted);">📋'+item.subtasks.length+'</span>':'')+(hasMemo?'<span style="font-size:9px;color:var(--muted);">📝</span>':'')+'</div>':'');}
    txt.addEventListener('click',()=>openDetail(idx));
    const acts=document.createElement('div');acts.style.cssText='display:flex;gap:2px;align-items:center;flex-shrink:0;';
    const chk=document.createElement('div');chk.style.cssText='width:18px;height:18px;border-radius:50%;border:1.5px solid '+(item.done?item.color:'#D0C8C0')+';background:'+(item.done?item.color:'transparent')+';display:flex;align-items:center;justify-content:center;cursor:pointer;';
    if(item.done)chk.innerHTML='<svg width="8" height="8" viewBox="0 0 8 8" fill="none"><path d="M1 4L3 6L7 2" stroke="white" stroke-width="1.4" stroke-linecap="round"/></svg>';
    chk.addEventListener('click',()=>{
      if(!item.isRepeat){item.done=!item.done;}
      else{const key=currentKey();if(!schedByDate[key])schedByDate[key]=[];let m=schedByDate[key].find(i=>i.templateId===item.templateId);if(m){m.done=!m.done;}else{schedByDate[key].push({...item,id:nextId++,done:true,isRepeat:false});}}
      save();redraw();
    });
    if(!item.isRepeat){
      const back=document.createElement('div');
      back.style.cssText='width:18px;height:18px;border-radius:50%;background:var(--pink-bg);display:flex;align-items:center;justify-content:center;cursor:pointer;';
      back.innerHTML='<svg width="10" height="10" viewBox="0 0 10 10" fill="none"><path d="M8 4.5C8 2.8 6.5 1.5 4.8 1.5c-1.1 0-2 .5-2.6 1.3" stroke="var(--pink)" stroke-width="1.2" stroke-linecap="round"/><path d="M1.5 1.5v2.3h2.3" stroke="var(--pink)" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"/><path d="M1.5 4.5A3.3 3.3 0 004.8 8 3.3 3.3 0 008 4.5" stroke="var(--pink)" stroke-width="1.2" stroke-linecap="round" stroke-dasharray="2 1"/></svg>';
      back.addEventListener('click',()=>returnToList(idx));
      acts.appendChild(chk);acts.appendChild(back);
    } else {
...
function returnToList(idx){
  const item=currentItems()[idx];if(!item||item.isRepeat)return;
  const key=currentKey();
  schedByDate[key]=(schedByDate[key]||[]).filter(i=>i.id!==item.id);
  tasks.push({id:nextId++,text:item.text,mins:item.mins,color:item.color,icon:item.icon||'other',repeat:'none',subtasks:item.subtasks||[],memo:item.memo||''});
  save();closeDetail();renderTasks();redraw();renderWeekStrip();
}
...
function openDetail(idx){
  ...
  const backBtn=document.getElementById('dp-back-btn');
  if(item.isRepeat){backBtn.style.display='none';}else{backBtn.style.display='';backBtn.onclick=()=>returnToList(idx);}
...
function requestDelete(idx){
  ...
  if(item.isRepeat||(item.hasRepeatBadge&&item.templateId)){
    deleteTarget={idx,item};document.getElementById('del-modal-name').textContent=item.text;document.getElementById('delete-modal').style.display='flex';
  } else {
    simpleDeleteTarget={type:'sched',idx,item};document.getElementById('sdel-modal-name').textContent=item.text;document.getElementById('simple-delete-modal').style.display='flex';
  }
}
function closeDeleteModal(){document.getElementById('delete-modal').style.display='none';deleteTarget=null;}
function deleteOnlyToday(){
  if(!deleteTarget)return;const{item}=deleteTarget;const key=currentKey();
  if(item.isRepeat){const tmpl=repeatTemplates.find(t=>t.id===item.templateId);if(tmpl){if(!tmpl.skipDates)tmpl.skipDates=[];tmpl.skipDates.push(key);}if(schedByDate[key])schedByDate[key]=(schedByDate[key]||[]).filter(i=>i.templateId!==item.templateId);}
  else{if(schedByDate[key])schedByDate[key]=(schedByDate[key]||[]).filter(i=>i.id!==item.id);if(item.hasRepeatBadge&&item.templateId){const tmpl=repeatTemplates.find(t=>t.id===item.templateId);if(tmpl){if(!tmpl.skipDates)tmpl.skipDates=[];tmpl.skipDates.push(key);}}}
  save();closeDeleteModal();closeDetail();renderTasks();redraw();renderWeekStrip();
}
function deleteAllRepeat(){
  if(!deleteTarget)return;const{item}=deleteTarget;const tid=item.templateId||item.id;
  repeatTemplates=repeatTemplates.filter(t=>t.id!==tid);
  Object.keys(schedByDate).forEach(k=>{schedByDate[k]=(schedByDate[k]||[]).filter(i=>i.templateId!==tid);});
  const key=currentKey();if(schedByDate[key])schedByDate[key]=(schedByDate[key]||[]).filter(i=>i.id!==item.id);
  save();closeDeleteModal();closeDetail();renderTasks();redraw();renderWeekStrip();
}
function requestSimpleDelete(type,id,idx){
  let name='';if(type==='task'){const t=tasks.find(t=>t.id===id);if(!t)return;name=t.text;}else{const items=currentItems();const item=items[idx];if(!item)return;name=item.text;}
  simpleDeleteTarget={type,id,idx};document.getElementById('sdel-modal-name').textContent=name;document.getElementById('simple-delete-modal').style.display='flex';
}
...
function confirmSimpleDelete(){
  if(!simpleDeleteTarget)return;const{type,id,idx}=simpleDeleteTarget;
  if(type==='task'){tasks=tasks.filter(t=>t.id!==id);save();renderTasks();}
  else if(type==='sched'){const item=currentItems()[idx];if(!item)return;const key=currentKey();schedByDate[key]=(schedByDate[key]||[]).filter(i=>i.id!==item.id);save();renderTasks();redraw();renderWeekStrip();}
  closeSimpleDeleteModal();closeDetail();
}
...
function doD(x,y){
  if(!drag||drag.type==='schedule')return;
  const allItems=currentItems();const item=allItems.find(i=>i.id===drag.itemId||(i.isRepeat&&'r'+i.templateId===String(drag.itemId)));if(!item)return;
  const mi=item.isRepeat?materialiseRepeat(item):findManualItem(item.id);if(!mi)return;
  if(drag.type==='move'){const dm=Math.round((y-drag.startY)/PX/5)*5;const nm=Math.max(0,Math.min(24*60-mi.mins,drag.startMin+dm));mi.startH=Math.floor(nm/60);mi.startMin=nm%60;showTip(x,y,mi.startH+':'+pad(mi.startMin));}
  else if(drag.type==='resizeTop'){const dm=Math.round((y-drag.startY)/PX/5)*5;const ns=Math.max(0,Math.min(drag.startTotal+drag.startMins-5,drag.startTotal+dm));mi.startH=Math.floor(ns/60);mi.startMin=ns%60;mi.mins=Math.max(5,drag.startMins-(ns-drag.startTotal));showTip(x,y,mi.startH+':'+pad(mi.startMin));}
  else if(drag.type==='resizeBot'){const dm=Math.round((y-drag.startY)/PX/5)*5;const mx=24*60-(mi.startH*60+mi.startMin);mi.mins=Math.max(5,Math.min(mx,drag.startMins+dm));showTip(x,y,endT(mi.startH,mi.startMin,mi.mins));}
  redraw();
}
...
// ---- Schedule drag (task list → timeline) ----
...
function mvG(x,y){const g=document.getElementById('ghost');g.style.left=x+'px';g.style.top=y+'px';document.querySelectorAll('.dz').forEach(z=>{const r=z.getBoundingClientRect();z.style.background=(x>=r.left&&x<=r.right&&y>=r.top&&y<=r.bottom)?'rgba(240,110,138,0.12)':''});}
...
function dropOn(h){
  if(!drag||drag.type!=='schedule')return;const t=tasks.find(t=>t.id===drag.taskId);if(!t)return;
  const key=currentKey();if(!schedByDate[key])schedByDate[key]=[];
  schedByDate[key].push({id:nextId++,text:t.text,mins:t.mins,color:t.color,icon:t.icon||'other',done:false,startH:h,startMin:0,repeat:'none',subtasks:JSON.parse(JSON.stringify(t.subtasks||[])),memo:t.memo||''});
  tasks=tasks.filter(t=>t.id!==drag.taskId);drag=null;
  save();renderTasks();redraw();renderWeekStrip();
  document.getElementById('tl-scroll').scrollTo({top:Math.max(0,topPx(h,0)-60),behavior:'smooth'});
}
...
function populateDayPicker(sel){const pk=document.getElementById('day-picker');pk.innerHTML='';DAY_NAMES.forEach((name,i)=>{const btn=document.createElement('div');const s=(sel||[]).includes(i);btn.dataset.dow=i;btn.style.cssText='width:28px;height:28px;border-radius:50%;border:1.5px solid '+(s?'var(--pink)':'var(--line)')+';background:'+(s?'var(--pink-bg)':'transparent')+';display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:11px;font-weight:500;color:'+(s?'var(--pink)':'var(--muted)')+';';btn.textContent=name;btn.addEventListener('click',()=>{const on=btn.style.border.includes('var(--pink)');btn.style.border='1.5px solid '+(on?'var(--line)':'var(--pink)');btn.style.background=on?'transparent':'var(--pink-bg)';btn.style.color=on?'var(--muted)':'var(--pink)';});pk.appendChild(btn);});}
...
function openAddModal(){editTarget={type:'new'};document.getElementById('modal-title').textContent='タスクを追加';document.getElementById('edit-name').value=document.getElementById('inp-name').value.trim();document.getElementById('edit-mins').value=document.getElementById('inp-mins').value;document.getElementById('edit-repeat').value='none';onRepeatChange();selectedColor=PALETTE[0];selectedIcon='work';modalSubtasks=[];renderColorPicker();renderIconPicker();renderModalSubtasks();document.getElementById('edit-memo').value='';populateDayPicker([]);populateTimePickers();document.getElementById('edit-modal').style.display='flex';}
function openEditModal(type,id,idx){
  let item;if(type==='task')item=tasks.find(t=>t.id===id);else if(type==='repeat')item=repeatTemplates.find(t=>t.id===id);else{const arr=currentItems();item=arr[idx];}if(!item)return;
  editTarget={type,id,idx};document.getElementById('modal-title').textContent='タスクを編集';
  document.getElementById('edit-name').value=item.text;
  const opts=[5,10,15,20,30,45,60,90,120,180,240];document.getElementById('edit-mins').value=opts.reduce((a,b)=>Math.abs(b-item.mins)<Math.abs(a-item.mins)?b:a);
  document.getElementById('edit-repeat').value=item.repeat||'none';onRepeatChange();
  selectedColor=item.color||PALETTE[0];selectedIcon=item.icon||'work';modalSubtasks=JSON.parse(JSON.stringify(item.subtasks||[]));
  renderColorPicker();renderIconPicker();renderModalSubtasks();
  document.getElementById('edit-memo').value=item.memo||'';
  populateDayPicker(item.repeatDays||[]);populateTimePickers();
  if(item.startH!==undefined)document.getElementById('repeat-start-h').value=item.startH;
  if(item.startMin!==undefined)document.getElementById('repeat-start-m').value=item.startMin;
  document.getElementById('edit-modal').style.display='flex';
}
function closeEditModal(){document.getElementById('edit-modal').style.display='none';editTarget=null;}
function applyEdit(){
  if(!editTarget)return;const name=document.getElementById('edit-name').value.trim();if(!name)return;
  const mins=parseInt(document.getElementById('edit-mins').value)||30;
  const repeat=document.getElementById('edit-repeat').value,repeatDays=repeat==='weekly'?getSelectedDays():[];
  const startH=parseInt(document.getElementById('repeat-start-h').value)||0,startMin=parseInt(document.getElementById('repeat-start-m').value)||0;
  const memo=document.getElementById('edit-memo').value,color=selectedColor,icon=selectedIcon;
  const subtasks=JSON.parse(JSON.stringify(modalSubtasks));
  if(editTarget.type==='new'){
    if(repeat!=='none'){repeatTemplates.push({id:nextId++,text:name,mins,color,icon,repeat,repeatDays,startH,startMin,subtasks,memo,repeatFromDate:currentKey()});}
    else{tasks.push({id:nextId++,text:name,mins,color,icon,repeat:'none',subtasks,memo});document.getElementById('inp-name').value='';}
  } else if(editTarget.type==='task'){
    const t=tasks.find(t=>t.id===editTarget.id);if(t){t.text=name;t.mins=mins;t.color=color;t.icon=icon;t.subtasks=subtasks;t.memo=memo;}
    if(repeat!=='none'){repeatTemplates.push({id:nextId++,text:name,mins,color,icon,repeat,repeatDays,startH,startMin,subtasks,memo,repeatFromDate:currentKey()});tasks=tasks.filter(t=>t.id!==editTarget.id);}
  } else if(editTarget.type==='repeat'){
    const tmpl=repeatTemplates.find(t=>t.id===editTarget.id);if(tmpl){tmpl.text=name;tmpl.mins=mins;tmpl.color=color;tmpl.icon=icon;tmpl.subtasks=subtasks;tmpl.memo=memo;tmpl.repeat=repeat;tmpl.repeatDays=repeatDays;tmpl.startH=startH;tmpl.startMin=startMin;}
  } else if(editTarget.type==='sched'){
    const items=currentItems();const srcItem=items[editTarget.idx];
    if(srcItem&&!srcItem.isRepeat){const mItem=findManualItem(srcItem.id);if(mItem){mItem.text=name;mItem.mins=mins;mItem.color=color;mItem.icon=icon;mItem.subtasks=subtasks;mItem.memo=memo;if(repeat!=='none')mItem.hasRepeatBadge=true;}}
    else if(srcItem&&srcItem.isRepeat){const mItem=materialiseRepeat(srcItem);if(mItem){mItem.text=name;mItem.mins=mins;mItem.color=color;mItem.icon=icon;mItem.subtasks=subtasks;mItem.memo=memo;}}
    if(repeat!=='none'){const tomorrow=new Date(currentDate.getTime()+86400000);const src=items[editTarget.idx];repeatTemplates.push({id:nextId++,text:name,mins,color,icon,repeat,repeatDays,startH:src?src.startH:startH,startMin:src?src.startMin:startMin,subtasks,memo,repeatFromDate:dateKey(tomorrow)});}
  }
  save();renderTasks();redraw();renderWeekStrip();closeEditModal();
}
...
function renderTasks(){
  ...
  tasks.forEach(t=>{
    ...
    el.innerHTML='<div style="display:flex;align-items:center;gap:5px;"><div style="width:22px;height:22px;border-radius:6px;background:'+t.color+';display:flex;align-items:center;justify-content:center;flex-shrink:0;">'+iconSvg(t.icon||'other','white')+'</div><div style="flex:1;min-width:0;"><div style="font-size:11px;font-weight:500;color:var(--text);line-height:1.3;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">'+esc(t.text)+'</div><div style="display:flex;gap:4px;align-items:center;"><span style="font-size:10px;color:var(--muted);">'+minsLabel(t.mins)+'</span>'+(hasSub?'<span style="font-size:10px;color:var(--muted);">📋'+t.subtasks.length+'</span>':'')+(hasMemo?'<span style="font-size:10px;color:var(--muted);">📝</span>':'')+'</div></div><div style="display:flex;gap:3px;flex-shrink:0;"><div data-edit="1" style="width:16px;height:16px;border-radius:4px;background:var(--pink-bg);display:flex;align-items:center;justify-content:center;cursor:pointer;"><svg width="8" height="8" viewBox="0 0 8 8" fill="none"><path d="M1 7L5 2L6.5 3.5L2.5 8H1V7Z" fill="var(--pink)"/></svg></div><div data-del="1" style="width:16px;height:16px;border-radius:4px;background:#FEE;display:flex;align-items:center;justify-content:center;cursor:pointer;"><svg width="9" height="9" viewBox="0 0 9 9" fill="none"><line x1="2" y1="2" x2="7" y2="7" stroke="#E05050" stroke-width="1.4" stroke-linecap="round"/><line x1="7" y1="2" x2="2" y2="7" stroke="#E05050" stroke-width="1.4" stroke-linecap="round"/></svg></div></div></div>';
  ...
  if(repeatTemplates.length){
    ...
    repeatTemplates.forEach(t=>{
      ...
      el.innerHTML='<div style="display:flex;align-items:center;gap:4px;"><div style="width:18px;height:18px;border-radius:5px;background:'+t.color+';display:flex;align-items:center;justify-content:center;flex-shrink:0;">'+iconSvg(t.icon,'white')+'</div><div style="flex:1;min-width:0;"><div style="font-size:11px;color:var(--text);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">'+esc(t.text)+'</div><div style="font-size:10px;color:var(--pink);">'+repeatLabel(t)+'</div></div><div style="display:flex;gap:3px;flex-shrink:0;"><div data-edit-repeat="'+t.id+'" style="width:16px;height:16px;border-radius:4px;background:rgba(240,110,138,0.15);display:flex;align-items:center;justify-content:center;cursor:pointer;"><svg width="8" height="8" viewBox="0 0 8 8" fill="none"><path d="M1 7L5 2L6.5 3.5L2.5 8H1V7Z" fill="var(--pink)"/></svg></div><div data-del-repeat="'+t.id+'" style="width:16px;height:16px;border-radius:4px;background:#FEE;display:flex;align-items:center;justify-content:center;cursor:pointer;"><svg width="9" height="9" viewBox="0 0 9 9" fill="none"><line x1="2" y1="2" x2="7" y2="7" stroke="#E05050" stroke-width="1.4" stroke-linecap="round"/><line x1="7" y1="2" x2="2" y2="7" stroke="#E05050" stroke-width="1.4" stroke-linecap="round"/></svg></div></div></div>';
      el.querySelector('[data-edit-repeat]').addEventListener('click',e=>{e.stopPropagation();openEditModal('repeat',t.id,null);});
      el.querySelector('[data-del-repeat]').addEventListener('click',e=>{e.stopPropagation();deleteTarget={idx:null,item:{isRepeat:true,templateId:t.id,text:t.text}};document.getElementById('del-modal-name').textContent=t.text;document.getElementById('delete-modal').style.display='flex';});
      list.appendChild(el);
    });
  }
}
